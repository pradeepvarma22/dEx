...
export default function Balance({ walletState, walletDispatch, tokenState, tokenDispatch, exchangeState, exchangeDispatch }) {
    ...
    function handlePayment_D_W(event, token) {
        ...
        if (paymentType == "Deposit") {
            ...
            if (token == "token1") {
                withdrawHandler(EXCHANGE_CONTRACT_ADDRESS, TOKEN_VARMA_CONTACT_ADDRESS, walletState.provider, token1TransferAmount)
            }
            else {
                const address_ = tokenState.navtokenstate == "USDT" ? (TOKEN_USDT_CONTACT_ADDRESS) : (TOKEN_INRT_CONTACT_ADDRESS)
                withdrawHandler(EXCHANGE_CONTRACT_ADDRESS, address_, walletState.provider, token2TransferAmount)
            }
        }

    }

    async function withdrawHandler(exchangeAddress, tokenAddress, provider, amount) {
        if (provider != null) {
            ...
            let txn;
            txn = await exchangeContract.withdrawToken(tokenAddress, amountToTransfer)
            txn = await txn.wait();
